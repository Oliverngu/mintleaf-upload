...
const AdminTodoApp: React.FC<AdminTodoAppProps> = ({ todos, loading, error, currentUser }) => {
  ...
  const { activeTodos, completedTodos } = useMemo(() => {
    const active = todos.filter(t => !t.isDone).sort((a,b) => (b.createdAt?.toDate().getTime() || 0) - (a.createdAt?.toDate().getTime() || 0));
    const completed = todos.filter(t => t.isDone).sort((a,b) => (b.completedAt?.toDate().getTime() || 0) - (a.completedAt?.toDate().getTime() || 0));

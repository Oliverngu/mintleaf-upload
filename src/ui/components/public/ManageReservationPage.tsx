...
import { translations } from '../../../lib/i1n';
...
const ManageReservationPage: React.FC<ManageReservationPageProps> = ({ token, allUnits }) => {
    ...
    useEffect(() => {
        ...
        if (allUnits.length > 0) {
            fetchBooking();
             ...
             setLoading(false);
             // Don't set an error immediately, wait for allUnits to potentially load.
             // If it remains empty after loading, the final check will show an error.
        }
    }, [token, allUnits]);
    ...
    if (!booking || !unit) return <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 text-center"><div className="bg-white p-8 rounded-lg shadow-md"><h2 className="text-xl font-bold text-red-600">Hiba</h2><p className="text-gray-800 mt-2">A foglalás nem található.</p></div></div>;;

...
const SliderControl: React.FC<{label: string, gradient: string, min: number, max: number, value: number, onChange: (v: number) => void}> = ({label, gradient, min, max, value, onChange}) => (
    <div>
        <label className="text-xs font-semibold text-gray-600">{label}</label>
        <div className="h-6 rounded-full" style={{ background: gradient }}>
            {/* FIX: Corrected the onChange handler to properly convert the event value to a number. */}
            <input type="range" min={min} max={max} value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-full h-full slider-thumb" />
        </div>
        <style>{`
